...
UnloggedTable='Unlogged_Census_Data'
...
def getSQLcmnds(rowlist):
	...
	for row in rowlist:
		valstr = row2vals(row)
		cmd = f"INSERT INTO {UnloggedTable} VALUES ({valstr});"
		cmdlist.append(cmd)
	return cmdlist
...
def load(conn, icmdlist):
	...
	with conn.cursor() as cursor:
		...
		for cmd in icmdlist:
			cursor.execute(cmd)
		cursor.execute(f"INSERT INTO {TableName} SELECT * FROM {UnloggedTable}")
